...
async function notifyAll(reviewData, sentiment, reply) {
  ...
  console.log('📣 Sending notifications via all channels...');

  const results = await Promise.allSettled([
    process.env.AISENSY_API_KEY && process.env.WHATSAPP_NUMBER
      ? sendWhatsApp(message, process.env.WHATSAPP_NUMBER)
      : Promise.resolve('WhatsApp skipped (no config)'),

    process.env.TELEGRAM_BOT_TOKEN && process.env.TELEGRAM_CHAT_ID
      ? sendTelegram(message, process.env.TELEGRAM_CHAT_ID)
      : Promise.resolve('Telegram skipped (no config)'),

    process.env.EMAIL_USER && process.env.EMAIL_PASS && process.env.EMAIL_TO
      ? sendEmail(
          `⭐ New ${starRating}-star review from ${authorName}`,
          message,
